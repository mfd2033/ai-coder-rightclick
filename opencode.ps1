<#
.SYNOPSIS
    opencode - 资源管理器右键菜单启动器

.DESCRIPTION
    由 Explorer 上下文菜单调用：先切换工作目录到目标文件夹，再执行 `opencode`。
...
if (-not (Get-Command opencode -ErrorAction SilentlyContinue)) {
    Write-Tag '未找到 opencode 命令。' 'Red'
    Write-Tag '请先安装 opencode，并确保其所在目录已加入 PATH。' 'DarkGray'
    exit 1
}
...
if ($null -ne $exitCode -and $exitCode -ne 0) {
    Write-Tag "opencode 退出码: $exitCode（非 0，请查看上方输出）" 'Yellow'
}
else {
