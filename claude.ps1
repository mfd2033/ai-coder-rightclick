...
.DESCRIPTION
    由 Explorer 上下文菜单调用：先切换工作目录到目标文件夹，再执行 `claude`。
...
if (-not (Get-Command claude -ErrorAction SilentlyContinue)) {
    Write-Tag '未找到 claude 命令。' 'Red'
    Write-Tag '请先安装 Claude Code CLI（例如：npm i -g @anthropic-ai/claude-code），并确保其所在目录已加入 PATH。' 'DarkGray'
    exit 1
}
...
if ($null -ne $exitCode -and $exitCode -ne 0) {
    Write-Tag "claude 退出码: $exitCode（非 0，请查看上方输出）" 'Yellow'
}
else {
